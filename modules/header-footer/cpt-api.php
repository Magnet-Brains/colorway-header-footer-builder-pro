...
class ColorwayHF_HeaderFooterBuilder_Api extends Core\Handler_Api {
    ...
    public function get_update(){
        ...
        $title = ($this->request['title'] == '') ? ('ColorwayHF Template #' . time()) : $this->request['title'];
        ...
        update_post_meta( $id, 'colorwayhf_template_activation', $activation );
        update_post_meta( $id, 'colorwayhf_template_type', $type );
        update_post_meta( $id, 'colorwayhf_template_condition_a', $condition_a );
        update_post_meta( $id, 'colorwayhf_template_condition_singular', $condition_singular );
        update_post_meta( $id, 'colorwayhf_template_condition_singular_id', $condition_singular_id );
            ...
            return [
                ...
                'data' => [
                    ...
                    'type_html' => (ucfirst($type) . (($activation == 'yes') 
                        ? ( '<span class="cw-headerfooter-status cw-headerfooter-status-active">'. esc_html__('Active', 'colorway-hf') .'</span>' ) 
                        : ( '<span class="cw-headerfooter-status cw-headerfooter-status-inactive">'. esc_html__('Inactive', 'colorway-hf') .'</span>' ))),
                ]
            ];
        }
    }
    ...
    public function get_get(){
        ...
        if($post != null){
            return [
                ...
                'activation' => get_post_meta($post->ID, 'colorwayhf_template_activation', true),
...
new ColorwayHF_HeaderFooterBuilder_Api();