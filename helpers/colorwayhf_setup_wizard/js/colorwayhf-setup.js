jQuery(document).ready(function ($) {
    ...
    jQuery('.colorway-import-content').on('click', function (e) {
        ...
        jQuery.ajax({
            type: "post",
            url: colorway_hf_setup_params.ajaxurl,
            data: {
            ...
            success: function (response) {
                ...
                if (response) {
                    ...
                    jQuery.ajax({
                        ...
                        url: colorway_hf_setup_params.ajaxurl,
                        data: {
                            action: 'colorway_theme_options',
                            data: '1',
                        },

                        success: function (responseMen) {
                            if (responseMen) {
                                console.log(responseMen);
...
var ColorwayHfWizard = (function ($) {
    ...
    function loading_content() {
        $('.colorway-hf-setup-content').block({
            message: null,
            overlayCSS: {
                background: '#fff',
                opacity: 0.6
    ...
    function PluginManager() {
                ...
                if (typeof response.url != 'undefined') {
                    ...
                    if (response.hash == current_item_hash) {
                        ...
                        jQuery.post(response.url, response, function (response2) {
                            process_current();
                            $current_node.find('span').text(response.message + colorway_hf_setup_params.verify_text);
                        }).fail(ajax_callback);
                    }
        ...
        function process_current() {
            if (current_item) {
                ...
                jQuery.post(colorway_hf_setup_params.ajaxurl, {
                    action: 'colorway_hf_setup_plugins',
                    wpnonce: colorway_hf_setup_params.wpnonce,
                    slug: current_item
        ...
        function find_next() {
            ...
            var $li = $('.colorway-hf-wizard-plugins li');
            $li.each(function () {
        ...
        return {
            init: function (btn) {
                $('.colorway-hf-wizard-plugins').addClass('installing');
                complete = function () {
                    loading_content();
    ...
    function ContentManager() {
        ...
        function process_current() {
            if (current_item) {
                ...
                if ($check.is(':checked')) {
                    ...
                    // process htis one!
                    jQuery.post(colorway_hf_setup_params.ajaxurl, {
                        action: 'colorway_hf_setup_content',
                        wpnonce: colorway_hf_setup_params.wpnonce,
        ...
        function find_next() {
            ...
            var $items = $('tr.colorway_hf_default_content');
            var $enabled_items = $('tr.colorway_hf_default_content input:checked');
            $items.each(function () {
                if (current_item == '' || do_next) {
        ...
        return {
            init: function (btn) {
                $('.colorway-hf-setup-pages').addClass('installing');
                $('.colorway-hf-setup-pages').find('input').prop("disabled", true);
                complete = function () {
                    loading_content();
                    window.location.href = btn.href;
                };
